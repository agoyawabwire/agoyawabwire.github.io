...
public class EggSaleAdapter extends RecyclerView.Adapter<EggSaleAdapter.ViewHolder> {

    private final List<EggSale> sales;
    private final OnSaleDeleteListener deleteListener;

    public interface OnSaleDeleteListener {
        void onDelete(EggSale sale);
    }

    public EggSaleAdapter(List<EggSale> sales, OnSaleDeleteListener deleteListener) {
        this.sales = sales;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        EggSale sale = sales.get(position);
        holder.tvDate.setText(sale.getDate());
        holder.tvDetail1.setText(sale.getSaleType() + ": " + sale.getQuantity());
